...
import { Button, toast, useConfig, usePayloadAPI, useRouteCache, useTranslation } from "@payloadcms/ui";
...
export const ScanActions = () => {
  ...
  const [{ data }, { setParams }] = usePayloadAPI(`${config.routes.api}/broken-link-checker/status`);
  ...
  const update = useCallback(() => {
    ...
    setElapsed(
      [duration.hours, duration.minutes, duration.seconds].map((v) => String(v ?? 0).padStart(2, "0")).join(":"),
    );
  }, [data]);

  const queueJob = useCallback(async () => {
    try {
      const response = await fetch(`${config.routes.api}/broken-link-checker/scan`, {
        method: "POST",
  ...
  if (data?.errors) {
    return (
      <Card>
        <p className={styles.error}>{t("brokenLinkChecker:somethingWentWrong")}</p>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <p>{data?.job ? t("brokenLinkChecker:scanInProgress", { elapsed }) : t("brokenLinkChecker:noScanInProgress")}</p>
